...
db.serialize(() => {
    ...
    // Ensure legacy pending accounts can sign in immediately
    db.run(`UPDATE users SET status = 'active' WHERE lower(coalesce(status, 'active')) = 'pending'`, [], (err) => {
        if (err) console.error('Could not normalize pending users', err.message);
    });
            ...
            db.run(sql, insertVals, function(err) {
                if (err) {
                    ...
                    if (message.includes('unique constraint failed: users.email')) {
                        return res.status(409).json({ error: 'Email already exists. Please sign in or use another email.' });
                    }
...
app.post('/api/auth/login', (req, res) => {
    ...
    db.all(sql, [identifier, identifier], async (err, candidates) => {
        ...
        try {
            ...
            if (status === 'pending') {
                return db.run(`UPDATE users SET status = 'active' WHERE id = ?`, [user.id], (updateErr) => {
