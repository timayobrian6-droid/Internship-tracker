...
function App() {
  ...
  const isBackendWakeupError = useCallback((error) => {
    if (!error) return false;
    const status = error?.response?.status;
    if (status === 502 || status === 503 || status === 504) return true;
    const msg = String(error?.message || '').toLowerCase();
    return msg.includes('network error') || msg.includes('timeout') || msg.includes('failed to fetch') || msg.includes('econnrefused');
  }, []);

  const checkBackendHealth = useCallback(async ({ retries = 2, delayMs = 1500 } = {}) => {
  ...
  useEffect(() => {
    if (!backendWakeMessage) return;
    const timer = setTimeout(() => {
      checkBackendHealth({ retries: 1, delayMs: 0 });
    }, 8000);
    return () => clearTimeout(timer);
  }, [backendWakeMessage, checkBackendHealth]);

  const wakeBanner = backendWakeMessage ? (
    <div style={{
      position: 'fixed',
      right: 16,
      top: 16,
      ...
      <button
        type="button"
        onClick={async () => {
          const ok = await checkBackendHealth({ retries: 2, delayMs: 1500 });
          if (ok && view === 'main') {
            loadData();
          }
        }}
        ...
        style={{
          ...
          fontWeight: 700,
          padding: '6px 10px',
          cursor: backendChecking ? 'not-allowed' : 'pointer',
          opacity: backendChecking ? 0.7 : 1
        }}
      >
        {backendChecking ? 'Checking...' : 'Retry now'}
      </button>
    </div>
  ) : null;
